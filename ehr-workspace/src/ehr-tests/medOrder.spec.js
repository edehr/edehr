...
describe('medOrders', () => {
  ...
  it('find metformin', () => {
    ...
    metformin.should.have.property('hasScheduledTimes')
    metformin.should.have.property('orderedDay')
    ...
    metformin.should.have.property('orderedTime')
    ...
    metformin.time1.should.equal('0811', 'med order is scheduled for 0811')
    metformin.time2.should.equal('2000')
    metformin.scheduledHours.should.have.length(2)
    metformin.scheduledHours[0].hour.should.equal(8, 'scheduledHours[0].hour')
    metformin.scheduledHours[1].hour.should.equal(20, '2000')
    metformin.hasScheduledTimes().should.equal(true, 'hasScheduledTimes')

    metformin.canOrderForDayTimeBlock(0,'0800').should.be.equal(false,'day too soon')
    metformin.canOrderForDayTimeBlock(1,'0800').should.be.equal(true, '1, 8000')
    metformin.isScheduled(0,'0800').should.be.equal(false,'day too soon')
    metformin.isScheduled(1,'0800').should.be.equal(true, '1, 8000')
    let stfdtb = metformin.getScheduledTimeForDayTimeBlock(1,'0800')
    should.exist(stfdtb)
    stfdtb.should.have.property('orderScheduleTime')
    stfdtb.orderScheduleTime.should.be.equal('0811', 'get scheduled time for time block 1 0800')
  })
