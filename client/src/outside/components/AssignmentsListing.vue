<template lang="pug">
  div(:class="$options.name")
    ...
    div(v-if="validationWarning")
      p(class="mis-configuration-warning") {{ validationWarning }}
...
export default {
  ...
  methods: {
    rowClass: function (item) {
      ...
      const isWarning = this.isItemMisconfigured(item) ? 'mis-configuration' : ''
    ...
    isItemMisconfigured (item) {
      ...
      const SEED_ID = '5e1dd3c8f3f225b4b6e937d8'
      return item.description === DESCRIPTION || 
      SEED_ID === item.seedDataObj._id || 
      item.name.includes('localhost assignment') // Is this a default title (is it the only one?)
    },
  },
  mounted: function () {
