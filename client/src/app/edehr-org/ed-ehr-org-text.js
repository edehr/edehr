export const EdOrgText = {
  ...
  servers: {
    title: 'Technical Security Measures', body: [
      'EdEHR servers operate on the stable Debian operating system and follow industry recommendations to lock down and secure site access. Administrative server access is only available via the SSH protocol and only to edehr.org administrators. All access is secured via SSL keys. No access is permitted to the root account nor via passwords.',

      'Students and instructors can only reach edehr.org through their learning management system (LMS). In particular, edehr.org does not have user accounts nor user passwords. Instead, it trusts the LMS to authenticate and authorize the users. All communications between the LMS and edehr.org are over secure HTTPS. The servers are protected by two firewalls. The first is a service provided by DigitalOcean on their network. The second is the firewall on the edehr.org server. Both limit internet traffic to web and admin protocols (HTTP, HTTPS and SSH) and block all other traffic.',

      'On the edehr.org server the web traffic is managed with a Nginx server. On edehr.org the Nginx server handles all of the encryption and decryption required for HTTPs. It serves the static HTML, Javascript and image files that provide the edehr.org service. Nginx also proxies (reroutes) all edehr.org API requests to the edehr.org API server.',
  ...
  userAccess: {
    title: 'User access to edehr.org services', body: [
      ...
      'System administrators can only access the server via SSH if their personal digital key has previously been placed on the server (i.e. they created the server or have been asked to help manage the server). Any of these users also need to be granted the privilege of working with "root" permissions. At the time of writing only one person has such access. The intention is to grow this to one other person who can provide necessary services to support the system if needed.',
      'Personal data is retained for one year on edEHR servers',
      'EdEHR has limited administration access that first requires the user enter the EdEHR via an LMS (so the user is authenticated). Then the user must provide a password that is only visible if the user also has gained access to the server which is blocked except for users explicitly configured to have access.'
    ]
  },
